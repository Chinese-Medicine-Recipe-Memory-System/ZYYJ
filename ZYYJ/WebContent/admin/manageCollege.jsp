	...
	<div class="row">
		...
		%>
			<tr>
				...
				<td><a href="#" data-toggle="modal" data-target="#modify-modal"><span class="glyphicon glyphicon-pencil" onclick="<%editid = testcollege.getCollegid();%>"> 修改</span></a></td>
...
<div class="modal fade" id="modify-modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
  	<div class="modal-dialog modal-md" role="document">
    	<div class="modal-content">
      		...
      		<div class="modal-body">
      			<form class="form-signin" action="<%=request.getContextPath()%>/CollegeUpdateAction" method="get">
      			<%
      				CollegeDB collegedb = new CollegeDB();
      				CollegeInfo collegeedit = collegedb.getCollegeById(editid);
      			%>
      				<input class="form-control" type="text" name="collegeid" placeholder="学院编号" value=<%=collegeedit.getCollegid() %>>
					<input class="form-control" type="text" name="collegename" placeholder="学院名称" value=<%=collegeedit.getCollegename() %>>
					<input class="form-control" type="text" name="collegenote" placeholder="备注" value=<%=collegeedit.getCollegedetail() %>>
