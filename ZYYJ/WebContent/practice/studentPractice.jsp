...
<div class="col-md-10 col-md-offset-1">
	...
	<div class="row">
		...
		<thead>
			<tr><th>记录编号</th><th>学生</th><th>关卡</th><th>是否通关</th><th>创建时间</th><th>分数</th><th>方案</th><th>科目</th></tr>
		</thead>
		<tbody>
			<%
			for(StudentPractice p : list){%>
				<tr><td><%=p.getPractice_id() %></td><td><%=p.getStudent_name() %></td><td><%=p.getCheckpoint_name() %></td>
				<td><%=p.getStatus()==1?"是":"否" %></td><td><%=p.getCreatetime() %></td><td><%=p.getScore() %></td>
				<td><%=p.getProgramme_name() %></td><td><%=p.getCourse_name() %></td></tr>
			<%}
			%>
		</tbody>
		</table>
